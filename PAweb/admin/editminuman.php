...
$id = $_GET['id'];
$result = mysqli_query($conn, "SELECT * FROM minuman WHERE id = $id");

$menu_minuman = [];
while ($row = mysqli_fetch_assoc($result)) {
    $menu_minuman[] = $row;
}
$menu_minuman = $menu_minuman[0];
...
if (isset($_POST['ubah'])) {
    ...
    if (!is_numeric($harga) || $harga < 0) {
        echo "
            <script>
                ...
                document.location.href = 'editminuman.php';
            </script>";
        exit;
    }
    ...
    if ($_FILES['gambar']['name']) {
        ...
        // Delete old image
        if (file_exists('uploads/' . $menu_minuman['gambar'])) {
            unlink('uploads/' . $menu_minuman['gambar']);
        }
        ...
        $sql = "UPDATE minuman SET nama='$nama', harga='$harga', gambar='$namaBaru' WHERE id=$id";
    } else {
    ...
    <div class="form-mhs">
        <form action="" method="post" enctype="multipart/form-data">
            <div class="input-field">
                <label class="label-field" for="nama">Nama minuman</label>
                <input type="text" name="nama" id="nama" value="<?php echo $menu_minuman['nama']; ?>" required>
            </div>
            <div class="input-field">
                <label class="label-field" for="harga">Harga</label>
                <input type="text" name="harga" id="harga" value="<?php echo $menu_minuman['harga']; ?>" required>
            </div>
            <div class="input-field">
